...
*/
    ...
    if (argc != 6) {
        std::cerr << "[grid.cpp] require argc: 6, input argc: " << argc << std::endl;
        std::cerr << "Format: program grid output_dir input_file cluster_size var_max" << std::endl;
        exit(-1);
    }
    ...
    int cluster_size = atoi(argv[4]);
    double var_max = atof(argv[5]);
    ...
    */
    ...
    points.erase(CGAL::hierarchy_simplify_point_set(points.begin(), points.end(), CGAL::Nth_of_tuple_property_map<0, MYPoint>(), cluster_size, var_max),
        points.end());
